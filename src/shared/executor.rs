...
pub fn start<R: Read + Send + 'static, W: Write + Send + 'static>(base_dir: String, ignores: RegexSet, reader: R, writer: W) {
    let lock = Arc::new(Mutex::new(0));
    let lock_clone = lock.clone();

    let base_dir_clone = base_dir.clone();
    let child_1 = thread::spawn(move || {
        run_local_watcher(base_dir_clone, ignores, writer, lock_clone);
    });

    let base_dir_clone = base_dir.clone();
    let child_2 = thread::spawn(move || {
        run_remote_listener(base_dir_clone, reader, lock);
    });
...
fn run_local_watcher<W: Write>(base_dir: String, ingores: RegexSet, writer: W, lock: Arc<Mutex<u8>>) {
